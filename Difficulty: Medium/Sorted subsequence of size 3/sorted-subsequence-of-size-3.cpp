...
class Solution {
public:
    std::vector<int> find3Numbers(std::vector<int> &arr) {
        int n = arr.size();
        
        if (n < 3) {
            return {};  // Return empty array since no such subsequence can exist
        }
        ...
        return {};  // No such triplet found, return empty array
    }
};
